                ...
                <div class="right-wap">
                    <div class="deal-data">
                        <dl>
                            ...
                            <c:forEach items="${paginationVO.list}" var="rechargeCord" varStatus="status">
                            <dd>
                                <div class="deal-time"><fmt:formatDate value="${rechargeCord.rechargeTime}"
                                                                       pattern="yyyy-MM-dd HH:mm:ss"/></div>
                                <div class="deal-name">${rechargeCord.rechargeDesc}</div>
                                <div class="deal-type" style="width:120px">充值</div>
                                <div class="deal-money">${rechargeCord.rechargeMoney}</div>
                                <c:choose>
                                <c:when test="${rechargeCord.rechargeStatus==0}">
                                <div class="deal-balance">
                                    充值中
                                </div>
                                </c:when>
                                <c:when test="${rechargeCord.rechargeStatus==1}">
                                <div class="deal-balance">
                                    充值成功
                                </div>
                                </c:when>
                                <c:when test="${rechargeCord.rechargeStatus==2}">
                                <div class="deal-balance">
                                    充值失败
                                </div>
                                </c:when>
                                ...
                                <div class="touzi_fenye" style="width:100%; text-align:center;line-height:30px;">
                                    共${paginationVO.total}条${totalPage}页　当前为第${currentPage}页　
                                    <a id="linkHomePage"
                                       href="${pageContext.request.contextPath}/loan/myRecharge">首页</a>
